...
===============================================================================
FILE: Knock_CRM_mng_v.1.60_Marketing.xlsm
Type: OpenXML
...
Public Sub Sheet_DentWeb_연동_Query()
    ...
    With Sheets(This_Sheet_Name)
        ...
        Set select_db_agent = New DB_Agent
        select_db_agent.DB_connect_str = "DSN=dentweb;uid=sa;pwd=Q3xzJiwpv2zC"
        ...
        sql_str = WorksheetFunction.Substitute(sql_str, ":param06", nid_str)
            ...
            'Else
                ...
                Select Case .Cells(row_idx, 2)
                    ...
                    Case "애드인"
                        ...
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx, 10) =
                        ' db_memo_1
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 11) = .Cells(row_idx, 5)
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx,12) =
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx,13) =
                   ...
                   Case "DS_10_LD_1", "DS_10_LD_2", "DS_10_LD_3", "DS_10_LD_4", "DS_10_LD_5", "DS_10_LD_6"
                     ' DB src
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 2) = .Cells(row_idx, 2)
                        ' DB src name
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 3) = WorksheetFunction.VLookup(.Cells(row_idx, 2), Range("CD_MAPPING_SRC_1_2"), 2, False)
                        ...
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx, 10) = Left(Right(.Cells(row_idx, 5), 2), 1)
                        ' db_memo_1
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 11) = .Cells(row_idx, 4) & "  |  " & .Cells(row_idx, 7) & "  |  " & .Cells(row_idx, 8)
                        ' db_memo_2
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx, 12) = .Cells(row_idx, 4)
                        ' db_memo_3
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx,13) =
                  ...
                  ' DB src 1
                   ...
                   Case "DS_T_V_HID"
                    ...
                    Case "케어랩스"
                        ...
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 4) = .Cells(row_idx, 7)
                        ' 이름
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 5) = .Cells(row_idx, 8)
                        ' 연락처
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 6) = .Cells(row_idx, 9)
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 14) = .Cells(row_idx, 9)
                        
                            
                        ' input 날짜
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 7) = Format(.Cells(row_idx, 24), "yyyyMMdd")
                        ' input 시간
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 8) = Format(.Cells(row_idx, 24), "hh:mm:ss")
                        ...
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx, 10) = Left(Right(.Cells(row_idx, 5), 2), 1)
                        ' db_memo_1
                        Sheets(Target_Sheet_Name).Cells(target_row_idx, 11) = .Cells(row_idx, 6) & "  |  " & _
                                                                                IIf(.Cells(row_idx, 15) = "", "", .Cells(row_idx, 15) & "  |  ") & _
                                                                                IIf(.Cells(row_idx, 16) = "", "", .Cells(row_idx, 16) & "  |  ") & _
                                                                                IIf(.Cells(row_idx, 17) = "", "", .Cells(row_idx, 17) & "  |  ")
                        ' db_memo_2
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx,12) =
                        ' db_memo_3
                        'Sheets(Target_Sheet_Name).Cells(target_row_idx,13) =
                        ...
                        Range(.Cells(row_idx, 7).Address & "," & _
                                .Cells(row_idx, 8).Address & "," & _
                                .Cells(row_idx, 9).Address & "," & _
                                .Cells(row_idx, 24).Address & "," & _
                                .Cells(row_idx, 6).Address & "," & _
                                .Cells(row_idx, 15).Address & "," & _
                                .Cells(row_idx, 16).Address & "," & _
                                .Cells(row_idx, 17).Address).Select
        ...
        While .Cells(row_idx, 2) <> ""
            ...
            If .Cells(2, 8) = "빅크래프트" Then
                ...
                If .Cells(row_idx, 12) >= 10 Or _
                    ...
                    InStr(.Cells(row_idx, 19), "결번") > 0 Then
...
' 실행: 현재 시트 C2(:param01), E2(:param02), N5~ 목록(또는 N5 CSV) → SQL!C77 치환 → 결과 B4부터 출력(B~L만)
Public Sub Run_SQL_C77_동적IN_출력()
    Const SQL_SHEET As String = "SQL"
    Const SQL_ADDR As String = "C77"
    Const HEADER_ROW As Long = 4      ' 헤더 B4
    Const START_COL As Long = 2       ' B=2
    Const MAX_LAST_COL As Long = 12   ' L=12
    Const MAX_ROWS As Long = 20000

    Dim ws As Worksheet, wsSQL As Worksheet
    Dim dtFrom As String, dtTo As String
    Dim sqlTpl As String, execSql As String
    Dim cn As Object, rs As Object
    Dim filter As String
    Dim f As Long, lastCol As Long, outCols As Long

    On Error GoTo EH
    Set ws = ActiveSheet
    Set wsSQL = ThisWorkbook.Worksheets(SQL_SHEET)

    ' 파라미터(C2/E2): YYYYMMDD
    dtFrom = Trim$(CStr(ws.Cells(2, 3).Value)) ' C2
    dtTo = Trim$(CStr(ws.Cells(2, 5).Value))   ' E2
    If Len(dtFrom) <> 8 Or Len(dtTo) <> 8 Then
        MsgBox "C2(:param01), E2(:param02)에 YYYYMMDD(예: 20250901)로 입력하세요.", vbExclamation
        Exit Sub
    End If

    ' SQL 템플릿 로드
    sqlTpl = CStr(wsSQL.Range(SQL_ADDR).Value)
    If Len(Trim$(sqlTpl)) = 0 Then
        MsgBox "SQL!C77에 쿼리 템플릿이 없습니다.", vbExclamation
        Exit Sub
    End If

    ' 파라미터 치환
    execSql = Replace(sqlTpl, ":param01", "'" & EscapeQuote(dtFrom) & "'")
    execSql = Replace(execSql, ":param02", "'" & EscapeQuote(dtTo) & "'")

    ' IN 필터 치환: N5:N999 세로목록 또는 N5 한칸 CSV
    filter = BuildDbSrc2Filter(ws, "N5:N999", "N5", "b.db_src_2")
    execSql = Replace(execSql, "/*DBSRC2_FILTER*/", filter)

    ' DB 실행 (Late Binding)
    Set cn = CreateObject("ADODB.Connection")
    cn.Open "DSN=knock_crm_real;uid=knock_crm;pwd=thsehdrnr123$"
    Set rs = CreateObject("ADODB.Recordset")
    rs.CursorLocation = 3 ' adUseClient
    rs.Open NoSemicolon(execSql), cn, 1, 1 ' adOpenKeyset, adLockReadOnly

    ' 출력 컬럼 계산(B~L만 사용)
    outCols = rs.Fields.Count
    lastCol = START_COL + outCols - 1
    If lastCol > MAX_LAST_COL Then
        lastCol = MAX_LAST_COL
        outCols = lastCol - START_COL + 1
        MsgBox "결과 필드가 L열을 초과하여 L열까지만 출력합니다.", vbInformation
    End If

    ' B4~L만 초기화(다른 열은 보존)
    ws.Range(ws.Cells(HEADER_ROW, START_COL), ws.Cells(HEADER_ROW + MAX_ROWS, MAX_LAST_COL)).ClearContents

    ' 헤더(B4~)
    For f = 0 To outCols - 1
        ws.Cells(HEADER_ROW, START_COL + f).Value = rs.Fields(f).Name
        ws.Cells(HEADER_ROW, START_COL + f).Font.Bold = True
    Next

    ' 데이터(B5~)
    If Not rs.EOF Then
        ws.Cells(HEADER_ROW + 1, START_COL).CopyFromRecordset rs
        ' L을 넘는 우측 데이터 잘라내기
        If rs.Fields.Count > outCols Then
            Dim dataLastRow As Long
            dataLastRow = ws.Cells(ws.Rows.Count, START_COL).End(xlUp).Row
            ws.Range(ws.Cells(HEADER_ROW + 1, START_COL + outCols), _
                     ws.Cells(dataLastRow, START_COL + rs.Fields.Count - 1)).ClearContents
        End If
    End If

    ' 폭 자동(B~L 범위 내)
    ws.Range(ws.Cells(HEADER_ROW, START_COL), ws.Cells(HEADER_ROW, lastCol)).EntireColumn.AutoFit
...
Private Function EscapeQuote(ByVal s As String) As String
    EscapeQuote = Replace(Trim$(s), "'", "''")
End Function

Private Function NoSemicolon(ByVal s As String) As String
    Dim t As String
    t = Trim$(s)
...
' N열 목록 → and ( b.db_src_2 in ('...') OR b.db_src_2 in ('...') ) 생성
Private Function BuildDbSrc2Filter(ByVal ws As Worksheet, _
                                   ByVal listRangeAddr As String, _
                                   ByVal altCellCsvAddr As String, _
                                   ByVal fieldExpr As String) As String
    Dim vals() As String
    Dim r As Range, c As Range, s As String, tmp As String

    ' 1) 세로목록 수집
    On Error Resume Next
    Set r = ws.Range(listRangeAddr)
    On Error GoTo 0
    If Not r Is Nothing Then
        For Each c In r.Cells
            s = Trim$(CStr(c.Value))
            If Len(s) > 0 Then AppendStrArray vals, CleanToken(s)
        Next
    End If

    ' 2) 비어있으면 N5 CSV 수집
    If UBoundSafe(vals) < 0 Then
        s = Trim$(CStr(ws.Range(altCellCsvAddr).Value))
        If Len(s) > 0 Then
            Dim parts() As String, i As Long
            parts = Split(s, ",")
            For i = LBound(parts) To UBound(parts)
                tmp = Trim$(parts(i))
                If Len(tmp) > 0 Then AppendStrArray vals, CleanToken(tmp)
            Next
        End If
    End If
    ...
    ' 4) Oracle IN 1000개 제한 회피(950개씩 분할)
    Dim clause As String
    clause = MakeInOrChunks(fieldExpr, vals, 950)
    BuildDbSrc2Filter = vbCrLf & "  and (" & clause & ")" & vbCrLf
End Function

Private Function CleanToken(ByVal s As String) As String
    CleanToken = Replace(Trim$(s), "'", "''")
End Function

Private Sub AppendStrArray(ByRef arr() As String, ByVal v As String)
    Dim n As Long
...
Private Function MakeInOrChunks(ByVal fieldExpr As String, _
                                ByRef vals() As String, _
                                ByVal chunkSize As Long) As String
    Dim i As Long, j As Long, ub As Long, piece As String, acc As String
    ub = UBound(vals)
    i = 0
    Do While i <= ub
        j = WorksheetFunction.Min(i + chunkSize - 1, ub)
        piece = BuildInList(fieldExpr, vals, i, j)
        If Len(acc) > 0 Then acc = acc & " OR "
        acc = acc & piece
        i = j + 1
    Loop
    MakeInOrChunks = acc
...
Private Function BuildInList(ByVal fieldExpr As String, _
                             ByRef vals() As String, _
                             ByVal i0 As Long, _
                             ByVal i1 As Long) As String
